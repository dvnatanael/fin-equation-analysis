...
class LinearProfile(AxialProfile):
    def d2Ac_dx2(self, x: np_arr_f64, y: np_arr_f64) -> np_arr_f64:
        return np.full_like(x, 2 * y[3][0] / self.L**2)


class ParabolicProfile(AxialProfile):
    def d2Ac_dx2(self, x: np_arr_f64, y: np_arr_f64) -> np_arr_f64:
        return 2 * (5 * x**2 / self.L**2 - 2) / self.L**2 * y[3][0]


class ExponentialProfile(AxialProfile):
    def d2Ac_dx2(self, x: np_arr_f64, y: np_arr_f64) -> np_arr_f64:
        return np.full_like(x, 4 * y[3])
...
class CosineProfile(AxialProfile):
    def d2Ac_dx2(self, x: np_arr_f64, y: np_arr_f64) -> np_arr_f64:
        return -((np.pi / self.L) ** 2) / 2 * y[3][0] * np.cos(np.pi * x / self.L)
